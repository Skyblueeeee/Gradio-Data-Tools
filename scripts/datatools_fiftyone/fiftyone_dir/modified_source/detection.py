    ...
    **kwargs,
    ...
    for sample in _samples.iter_samples(progress=True):
        ...
        for doc in docs:
            ...
            # by Joyphy
            if config.method == "cocofitow":
                tp, fp, fn = _tally_matches_coco_fitow(doc_matches)
            else:
...
def _parse_config(pred_field, gt_field, method, is_temporal, **kwargs):
    ...
    # by Joyphy
    if method == "cocofitow":
        from .coco_fitow import COCOFitowEvaluationConfig
        
        return COCOFitowEvaluationConfig(pred_field, gt_field, **kwargs)
    # Joyphy
...
# by Joyphy
def _tally_matches_coco_fitow(matches):
    tp = 0
    fp = 0
    fn = 0
