...
# by Joyphy
class SelectSql(ViewStage):
    import sqlite3
    import re
    import os
    def __init__(self, sql_sentence, sqlite3_db_path, ordered=False):
        def regexp(expr, item):
            reg = self.re.compile(expr)
        ...
        def db_connection(sqlite3_db_path):
            try:
                con = self.sqlite3.connect(sqlite3_db_path)
                con.create_function("REGEXP", 2, regexp)
                return con
            except Exception as e:
                print(f"数据库连接失败：{str(e)}")

        if not self.os.path.exists(sqlite3_db_path):
            raise ValueError(f"sqlite3数据库路径<{sqlite3_db_path}>不存在, 请检查地址(使用绝对地址)")

        conn = db_connection(sqlite3_db_path)
        cursor = conn.cursor()

        cursor.execute(f"select id from ({sql_sentence})")

        self._sample_ids = list(zip(*cursor.fetchall()))[0]
        self._bools = False
    ...
    @property
    def sql_sentence(self):
        return self._sql_sentence

    @property
    def sqlite3_db_path(self):
        return self._sqlite3_db_path
    ...
    def _kwargs(self):
        return [["sql_sentence", self._sql_sentence], ["sqlite3_db_path", self._sqlite3_db_path], ["ordered", self._ordered]]
        ...
        return [
            {
                "name": "sql_sentence",
                "type": "str",
                "default": "select * from imageInfo limit 50",
                "placeholder": "select * from imageInfo limit 50",
            },
            {
                "name": "sqlite3_db_path",
                "type": "str",
                "default": r"/root/workspace/fo_data/sqlite3/fitow_fiftyone.db",
                "placeholder": r"添加sqlite3数据库绝对路径",
            },
            {
                "name": "ordered",
                "type": "bool",
