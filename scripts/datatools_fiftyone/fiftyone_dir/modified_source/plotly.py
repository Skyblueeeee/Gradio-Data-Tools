...
class InteractiveHeatmap(PlotlyInteractivePlot):
    ...
    def _update_heatmap(self, cells, Z_active, Z_bg, zlim):
        ...
        with self._widget.batch_update():
            ...
            # by Joyphy
            indexes = (Z_active != None)
            if indexes.sum() == indexes.size:
                self._bgw.z = Z_bg
            else:
                Z_temp = Z_bg.astype(np.object0)
                Z_temp[indexes] = None
                self._bgw.z = Z_temp
            # Joyphy

            # self._bgw.z = Z_bg # by Joyphy
    ...
    def _make_heatmap(self):
        ...
        selected = go.Heatmap(
            ...
            zmin=self.zlim[0],
            zmax=self.zlim[1],
            # colorscale=self.colorscale, # by Joyphy
            colorscale=[[0.0, "green"], [1.0, "green"]], # by Joyphy
            showscale=False,
            hoverinfo="skip",
        )
        ...
        bg = go.Heatmap(
            ...
            # opacity=self.bg_opacity, # by Joyphy
            hovertemplate=hovertemplate,
        )
