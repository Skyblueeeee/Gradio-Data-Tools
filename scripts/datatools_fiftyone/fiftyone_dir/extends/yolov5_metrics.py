...
def process_batch(detections, labels, iouv):
    ...
    if x[0].shape[0]:
        ...
        correct[matches[:, 1].astype(np.int32)] = matches[:, 2:3] >= iouv
    return correct
...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=()):
    ...
    # Find unique classes
    unique_classes = np.unique(target_cls).astype(np.int32)
    with open(os.path.join(save_dir, 'unique_classes.txt'), 'w') as fp:
        json.dump([(key, names[key]) for key in unique_classes.tolist()], fp, indent=4)
    ...
    return p[:, i], r[:, i], ap, f1[:, i], unique_classes.astype(np.int32)
...
def plot_pr_curve(px, py, ap, save_dir='pr_curve.png', names=()):
    ...
    path = Path(save_dir)
    with open(os.path.join(path.parent, 'unique_classes.txt'), 'r') as fp:
        unique_classes = json.load(fp)

    if 0 < len(unique_classes) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            j = int(unique_classes[i][0])
            ax.plot(px, y, linewidth=1, label=f'{names[j]} {ap[i, 0]:.3f}')  # plot(recall, precision)
    else:
...
def plot_mc_curve(px, py, save_dir='mc_curve.png', names=(), xlabel='Confidence', ylabel='Metric'):
    ...
    if 0 < len(unique_classes) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py):
            j = int(unique_classes[i][0])
            ax.plot(px, y, linewidth=1, label=f'{names[j]}')  # plot(confidence, metric)
    else:
